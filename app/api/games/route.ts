...
export async function GET(req:Request){
  ...
  const mode=new URL(req.url).searchParams.get('mode')||'word-match';await dbConnect();
  const words=await Vocabulary.aggregate([{$match:{published:true,'zhTWDefinitions.0':{$exists:true},'englishDefinitions.0':{$exists:true}}},{$sample:{size:24}},{$project:{word:1,zhTWDefinitions:1,englishDefinitions:1,examples:1,morphology:1}}]);
  ...
  const rounds=words.slice(0,10).map((w:any,idx:number)=>{
    let prompt=w.word,answer=w.zhTWDefinitions?.[0]||'';
    ...
    const answerPool=mode==='definition-match'||mode==='spelling-challenge'?words.filter((_:any,i:number)=>i!==idx).map((x:any)=>x.word):words.filter((_:any,i:number)=>i!==idx).map((x:any)=>x.zhTWDefinitions?.[0]).filter(Boolean);
...
export async function POST(req:Request){
  ...
  const b=await req.json();const game=allowedGames.has(b.mode)?b.mode:'word-match';const correctCount=Math.max(0,Number(b.correct)||0);const wrongCount=Math.max(0,Number(b.wrong)||0);const total=Math.max(1,correctCount+wrongCount);const score=Math.max(0,Number(b.score)||0);const xpEarned=Math.min(200,Math.floor(score/20)+correctCount*2);
