...
export async function POST(req:Request){
  ...
  await dbConnect();const found:any[]=await Question.find({_id:{$in:ids},published:true}).lean();const byId=new Map(found.map((q:any)=>[String(q._id),q]));const qs:any[]=ids.map((id:string)=>byId.get(id)).filter(Boolean);
  ...
  const attempt=await ExamAttempt.create({userId,examType,questionIds:qs.map((q:any)=>q._id),answers:review.map((r:any)=>({questionId:r.questionId,answer:r.selected,correct:r.correct})),score,accuracy,durationSeconds:Math.max(0,Number(body.durationSeconds)||0),flaggedQuestionIds:flagged,completedAt:new Date()});
