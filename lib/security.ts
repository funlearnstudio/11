...
export async function rateLimit(req:Request,scope:string,limit:number,windowSeconds:number){
  ...
  const doc=await RateLimit.findOneAndUpdate({key},{$inc:{count:1},$setOnInsert:{expiresAt}},{upsert:true,new:true,setDefaultsOnInsert:true});
  return {allowed:doc.count<=limit,remaining:Math.max(0,limit-doc.count),retryAfterSeconds:Math.max(1,Math.ceil((expiresAt.getTime()-now)/1000))};
}
